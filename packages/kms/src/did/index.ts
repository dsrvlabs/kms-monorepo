...
export async function verifyDid(jwt: string, audience: string): Promise<JWTVerified | null> {
  try {
...
export async function createDid(path: BIP44, mnemonic: string, name: string): Promise<string> {
  ...
  if (child.privateKey) {
    ...
    const didDocument: DIDDocument = createDidDoc(Buffer.from(child.publicKey).toString('base64'));
