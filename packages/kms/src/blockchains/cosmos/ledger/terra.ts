...
export class LEDGER {
  ...
  static async signTx(path: BIP44, transport: Transport, parsedTx: any): Promise<SignedTx> {
    const instance = new TerraApp(transport);
    ...
    const newSig = Secp256k1Signature.fromDer(new Uint8Array(response.signature.data));
