...
export class KEYSTORE {
  static async getAccount(node: BIP32Interface | string, prefix: string): Promise<Account> {
    const privateKey =
  ...
  static async signMessage(node: BIP32Interface | string, _prefix: string, msg: Message) {
    const privateKey =
      typeof node !== 'string' ? node.privateKey : Buffer.from(node.replace('0x', ''), 'hex');
