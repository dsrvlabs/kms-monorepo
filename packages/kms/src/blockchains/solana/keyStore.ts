...
export class KEYSTORE {
  ...
  private static getKeypair(seed: Buffer | string, path?: BIP44): Keypair {
    const temp = typeof seed === 'string' ? Buffer.from(seed.replace('0x', ''), 'hex') : seed;
    const key = path ? KEYSTORE.getPrivateKey(temp, path).replace('0x', '') : temp.toString('hex');
  ...
  static signTx(seed: Buffer | string, serializedTx: string, path?: BIP44): SignedTx {
    const payer = KEYSTORE.getKeypair(seed, path);
    const transaction = Transaction.from(Buffer.from(serializedTx.replace('0x', ''), 'hex'));
    transaction.sign(payer);
    return {
      hash: transaction.signature ? base58.encode(Uint8Array.from(transaction.signature)) : '',
      serializedTx,
  ...
  static async signMessage(seed: Buffer | string, msg: Message, path?: BIP44) {
    ...
    const result = nacl.sign.detached(
      Buffer.from(msg.data.replace('0x', ''), msg.data.startsWith('0x') ? 'hex' : 'utf8'),
      payer.secretKey,
    );
    return {
      msg,
